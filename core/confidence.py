...
from core.flaky_bank import load_flaky_snapshot
...
def compute_composite_confidence(
    ...
    negative_evidence_score, negative_evidence = _negative_evidence_score(page_info, page_model)
    ...
    score = round(_clamp(sum(value * weight for _, value, weight in weighted_items)), 2)
...
def _negative_evidence_score(page_info: dict, page_model: dict) -> tuple[float, dict]:
    fingerprint = page_info.get("page_fingerprint", {})
    tracked = {
    ...
    for label, (fingerprint_key, model_signal) in tracked.items():
        if fingerprint_key in fingerprint:
            ...
            positive.append(label)
    score = _clamp((min(len(negative), 6) * 0.12) + (min(explicit_count, len(tracked)) / max(len(tracked), 1) * 0.28) + 0.2)
    return score, {
